...
다음 규칙을 준수하세요:
1. 시간표의 각 Course 객체는 다음 필드를 가집니다: id(string), name(string), professor(string), room(string), credits(number), day("월"|"화"|"수"|"목"|"금"), startHour(9~17 정수), duration(1~3 정수), colorIndex(0~5 정수), failed(boolean optional)
2. 같은 과목이 여러 요일에 있을 수 있습니다 (e.g., 월수 각각 별도 Course 객체)
3. 시간 충돌이 없도록 하세요
4. 새 과목 추가 시 현실적인 한국 대학 과목명, 교수명, 강의실을 사용하세요
5. id는 "새과목이름-요일" 형식으로 고유하게 생성하세요
6. 응답은 다음 JSON 형식으로만 출력하세요 (다른 텍스트 없이):
  ...
  "updatedTimetable": [/* Course 객체 배열 전체 */]
      ...
      try {
        ...
        return NextResponse.json({
          ...
          updatedTimetable: parsed.updatedTimetable ?? currentTimetable,
        });
      } catch {
        return NextResponse.json({
          reply: '시간표를 분석했는데 응답 형식에 문제가 생겼어요. 다시 시도해 주세요.',
...
function getDemoReply(
  message: string,
  currentTimetable: Record<string, unknown>[]
) {
  const msg = message.toLowerCase();

  if (msg.includes('금공강') || msg.includes('금요일')) {
    const filtered = currentTimetable.filter(
      (c: Record<string, unknown>) => c.day !== '금'
    );
    return {
      reply:
  ...
  if (msg.includes('1교시') || msg.includes('아침')) {
    const filtered = currentTimetable.filter(
      (c: Record<string, unknown>) => (c.startHour as number) !== 9
    );
    return {
      reply:
        '1교시 제거 완료! ☀️ 9시 수업을 모두 없앴어요. 이제 아침잠을 마음껏 즐기세요. (데모 모드)',
      timetable: filtered,
    };
  }
